...
import re
import lettuce_bunch.special
from utils import onfailure, debug, conf
...
@step(u'I create nova network "(.*?)" labeled "(.*?)", bridge_interface "(.*?)", vlan id "(.*?)"')
def create_nova_network(step, cidr, label, bridge, vlan):
    step_assert(step).assert_true(utils.nova_manage.create_network_new(cidr, label, bridge, vlan))
...
@step(u'I get EC2 credentials')
def ec2_get_keys(step):
    utils.nova_cli.ec2_get_keys(bunch_working_dir)
    utils.keystone.ec2_get_env()
    return True
...
def register_images_glance(step, name, owner, disk, ram, kernel):
    step_assert(step).assert_true(utils.glance_cli.vm_image_all_register(name, owner,
...
def register_images_glance(step, name, owner, disk):
    step_assert(step).assert_true(utils.glance_cli.vm_image_register(name, owner,
                                                                    os.path.join(bunch_working_dir,disk)))
@step(u'using glance I see VM image "(.*?)" registered')
def image_registered_glance(step, name):
    step_assert(step).assert_true(utils.glance_cli.vm_image_registered(name))
...
def register_all_images_nova_manage(step, name, owner, disk):
...
@step(u'using nova-manage I see VM image "(.*?)" registered')
def image_registered_nova_manage(step, name):
    pass # nova-manage have no command to list images


##--  Nova2ools
### TODO  ### TODO  ### TODO  ### TODO Need checking
@step(u'using nova2ools I register VM image "(.*?)" for owner "(.*?)" using disk "(.*?)"')
def register_images_nova2ools(step, name, owner, disk):
    step_assert(step).assert_true(utils.nova2ools_cli.vm_image_register(name, owner,
                                                                    os.path.join(bunch_working_dir, disk), None, None))
### TODO  ### TODO  ### TODO  ### TODO Need checking
@step(u'using nova2ools I register VM image "(.*?)" for owner "(.*?)" using disk "(.*?)", ram "(.*?)", kernel "(.*?)"')
def register_all_images_nova2ools(step, name, owner, disk, ram, kernel):
    step_assert(step).assert_true(utils.nova2ools_cli.vm_image_register(name, owner, os.path.join(bunch_working_dir, disk),
                                                                    os.path.join(bunch_working_dir, ram),
                                                                    os.path.join(bunch_working_dir, kernel)))

@step(u'using nova2ools I see VM image "(.*?)" registered')
def image_registered_nova2ools(step, name):
    step_assert(step).assert_true(utils.nova2ools_cli.vm_image_registered(name))


##--  Euca2ools
### TODO  ### TODO  ### TODO  ### TODO  
@step(u'using euca2ools I register VM image "(.*?)" for owner "(.*?)" using disk "(.*?)"')
def register_images_euca(step, name, owner, disk):
    step_assert(step).assert_true(utils.euca_cli.vm_image_register(name, owner,
                                                                    os.path.join(bunch_working_dir,disk), None, None))
### TODO  ### TODO  ### TODO  ### TODO  
@step(u'using euca2ools I register VM image "(.*?)" for owner "(.*?)" using disk "(.*?)", ram "(.*?)", kernel "(.*?)"')
def register_all_images_euca(step, name, owner, disk, ram, kernel):
    step_assert(step).assert_true(utils.euca_cli.vm_image_all_register(name, owner, os.path.join(bunch_working_dir, disk),
                                                                    os.path.join(bunch_working_dir,ram),
                                                                    os.path.join(bunch_working_dir, kernel)))

@step(u'using euca2ools I see VM image "(.*?)" registered')
def image_registered_euca(step, name):
    step_assert(step).assert_true(utils.euca_cli.vm_image_registered(name))




@step(u'I register VM image "(.*?)" using preuploaded image name "(.*?)"')
def deregister_all_images(step, image_name, os_name):
    step_assert(step).assert_true(utils.nova_cli.vm_image_register(image_name, os_name))
...
@step(u'I deregister VM image "(.*?)"')
def deregister_all_images(step, image_name):
    step_assert(step).assert_true(utils.glance_cli.deregister_image(image_name))
...
@step(u'I spawn VM instance "(.*?)" using image named "(.*?)", flavor named "(.*?)" and keypair named "(.*?)"')
def start_vm_instance1(step, name,image_name, flavor_name, key_name):
    step_assert(step).assert_true(utils.nova_cli.start_vm_instance1(name, image_name, flavor_name, key_name))

@step(u'I spawn VM instance "(.*?)" using image named "(.*?)", flavor named "(.*?)" and keypair named "(.*?)" in security group "(.*?)"')
def start_vm_instance2(step, name,image_name, flavor_name, key_name, secgroup):
    step_assert(step).assert_true(utils.nova_cli.start_vm_instance1(name, image_name, flavor_name, key_name, secgroup))
...
def suspend_instance(step, vm_name):
    step_assert(step).assert_true(utils.nova_cli.suspend_vm_instance(vm_name))

@step(u'VM instance "(.*?)" is suspended within "(.*?)" seconds')
def wait_instance_suspended(step, vm_name, timeout):
    step_assert(step).assert_true(utils.nova_cli.wait_instance_state(vm_name, 'SUSPENDED', int(timeout)))


@step(u'I resume VM instance "(.*?)"')
def resume_instance(step, vm_name):
    step_assert(step).assert_true(utils.nova_cli.resume_vm_instance(vm_name))

@step(u'I reboot VM instance "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def reboot_vm(step, vm_name):
    step_assert(step).assert_true(utils.nova_cli.reboot_vm_instance(vm_name))
...
def create_volume(step, volume_name, volume_size, volume_zone):
    step_assert(step).assert_true(utils.nova_cli.volume_create(name=volume_name, size=volume_size, zone=volume_zone))

@step(u'I remove volume "(.*?)"')
def remove_volume(step, volume_name):
    step_assert(step).assert_true(utils.nova_cli.volume_delete(volume_name=volume_name))

@step(u'I remove ALL volumes')
def remove_all_volumes(step):
    step_assert(step).assert_true(utils.nova_cli.volume_delete_all())

@step(u'volume "(.*?)" comes up within "(.*?)" seconds')
def check_volume_comes_up(step, volume_name, timeout):
    step_assert(step).assert_true(utils.nova_cli.wait_volume_comes_up(volume_name=volume_name, timeout=timeout))

@step(u'I see volume "(.*?)" available')
def check_volume_available(step, volume_name):
    step_assert(step).assert_equals(utils.nova_cli.get_volume_status(volume_name=volume_name)['status'], 'available', 'Volume is not available')

@step(u'I see volume "(.*?)" removed')
def check_volume_removed(step, volume_name):
    step_assert(step).assert_true(utils.nova_cli.check_volume_deleted(volume_name=volume_name))

@step(u'I attach volume "(.*?)" to VM instance "(.*?)" as device "(.*?)"')
def attach_volume(step, volume_name, instance_name, volume_dev):
    step_assert(step).assert_true(utils.nova_cli.volume_attach(instance_name=instance_name, dev=volume_dev, volume_name=volume_name))

@step(u'I see volume "(.*?)" attached to VM instance "(.*?)"')
def check_volume_attached(step, volume_name, instance_name):
    step_assert(step).assert_true(utils.nova_cli.volume_attached_to_instance(instance_name=instance_name, volume_name=volume_name))

@step(u'I detach volume "(.*?)" attached to VM instance "(.*?)"')
def dettach_volume(step, volume_name, instance_name):
    step_assert(step).assert_true(utils.nova_cli.volume_detach(volume_name=volume_name, instance_name=instance_name))

@step(u'wait volume "(.*?)" state "(.*?)" for "(.*?)" seconds')
def check_volume_comes_up(step, volume_name, state, timeout):
    step_assert(step).assert_true(utils.nova_cli.wait_volume_state(volume_name=volume_name, state=state, timeout=timeout))



@step(u'using euca tools I create volume "(.*?)" with size of "(.*?)" in zone "(.*?)"')
def create_volume(step, volume_name, volume_size, volume_zone):
    step_assert(step).assert_true(utils.euca_cli.volume_create(name=volume_name, size=volume_size, zone=volume_zone))

@step(u'using euca tools I remove volume "(.*?)"')
def remove_volume(step, volume_name):
    step_assert(step).assert_true(utils.euca_cli.volume_delete(volume_name=volume_name))

@step(u'using euca tools I remove ALL volumes')
def remove_all_volumes(step):
    step_assert(step).assert_true(utils.euca_cli.volume_delete_all())

@step(u'using euca tools volume "(.*?)" comes up within "(.*?)" seconds')
def check_volume_comes_up(step, volume_name, timeout):
    step_assert(step).assert_true(utils.euca_cli.wait_volume_comes_up(volume_name=volume_name, timeout=timeout))

@step(u'using euca tools I see volume "(.*?)" available')
def check_volume_available(step, volume_name):
...
@step(u'using euca tools I see volume "(.*?)" removed')
def check_volume_removed(step, volume_name):
    step_assert(step).assert_true(utils.euca_cli.check_volume_deleted(volume_name=volume_name))

@step(u'using euca tools I attach volume "(.*?)" to VM instance "(.*?)" as device "(.*?)"')
def attach_volume(step, volume_name, instance_name, volume_dev):
...
@step(u'using euca tools I see volume "(.*?)" attached to VM instance "(.*?)"')
def check_volume_attached(step, volume_name, instance_name):
...
@step(u'using euca tools I detach volume "(.*?)"')
...
@step(u'I run keystone-manage db_sync')
def keystone_db_sync(step):
    step_assert(step).assert_true(utils.keystone_manage.db_sync())
...
##  ----------------------------------------------------------
##
@step(u'I create tenant "(.*?)"')
def kclient_create_tenant(step, tenant):
    step_assert(step).assert_true(utils.keystone.tenant_create(tenant))

@step(u'I delete tenant "(.*?)"')
def kclient_delete_tenant(step, tenant):
    step_assert(step).assert_true(utils.keystone.tenant_delete(tenant))

@step(u'tenant "(.*?)" exist')
def kclient_check_tenant_exist(step, tenant):
    step_assert(step).assert_true(utils.keystone.tenant_check(tenant, exist=True))



@step(u'I create role "(.*?)"')
def kclient_create_role(step, role):
    step_assert(step).assert_true(utils.keystone.role_create(role))

@step(u'I delete role "(.*?)"')
def kclient_delete_role(step, role):
    step_assert(step).assert_true(utils.keystone.role_delete(role))

@step(u'role "(.*?)" exist')
def kclient_check_role_exist(step, role):
    step_assert(step).assert_true(utils.keystone.role_check(role, exist=True))





@step(u'I create user "(.*?)", password "(.*?)"')
def kclient_create_user(step, user, password):
    step_assert(step).assert_true(utils.keystone.user_create(user, password))

@step(u'I create user "(.*?)", password "(.*?)", tenant "(.*?)"')
def kclient_create_user_tenant(step, user, password, tenant):
    step_assert(step).assert_true(utils.keystone.user_create(user, password, tenant))

@step(u'I delete user "(.*?)"')
def kclient_delete_user(step, user):
    step_assert(step).assert_true(utils.keystone.user_delete(user))

@step(u'user "(.*?)" exist')
def kclient_check_user_exist(step, user):
    step_assert(step).assert_true(utils.keystone.user_check(user, exist=True))





@step(u'I add role "(.*?)" for user "(.*?)" in tenant "(.*?)"')
def kclient_add_user_role(step, role, user, tenant):
    step_assert(step).assert_true(utils.keystone.role_add_user(role, user, tenant))

@step(u'I create service named "(.*?)", type "(.*?)", description "(.*?)"')
def kclient_create_service(step, name, stype, description):
    step_assert(step).assert_true(utils.keystone.service_create(name, stype, description))

@step(u'I delete service named "(.*?)"')
def kclient_delete_service(step, name):
    step_assert(step).assert_true(utils.keystone.service_delete(name))

@step(u'service "(.*?)" exist')
def kclient_check_role_exist(step, service):
    step_assert(step).assert_true(utils.keystone.service_check(service, exist=True))
...
@step(u'I create volume for VM instance "(.*)" with size "(.*)" as device "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def create_clean_volume(step, vm_name, size, device):
    step_assert(step).assert_true(utils.nova2ools_cli.local_volume_create(vm_name, size, device))


@step(u'device "(.*)" is attached to VM instance "(.*)" in "(.*)" seconds')
@onfailure(debug.save.log('nova/nova-compute.log'))
def volume_is_attached(step, device, vm_name, timeout):
    step_assert(step).assert_true(utils.nova2ools_cli.wait_local_volume_state(vm_name, device, 'in-use', timeout))


@step(u'I can\'t create volume for VM instance "(.*)" with size "(.*)" as device "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def cant_create_volume(step, vm_name, size, device):
    step_assert(step).assert_false(utils.nova2ools_cli.local_volume_create(vm_name, size, device))


@step(u'I create snapshot from device "(.*)" of VM instance "(.*)" and name it "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def create_snapshot(step, device, vm_name, snapshot_name):
    step_assert(step).assert_true(utils.nova2ools_cli.local_volume_snapshot(vm_name, device, snapshot_name))

@step(u'snapshot "(.*)" is active within "(.*)" seconds')
@onfailure(debug.save.log('nova/nova-compute.log'))
def snapshot_is_ok(step, name, timeout):
    step_assert(step).assert_true(utils.nova2ools_cli.wait_local_volume_snapshot_state(name, 'ACTIVE', timeout))

@step(u'I create volume for VM instance "(.*)" from snapshot "(.*)" as device "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def create_volume_from_snapshot(step, vm_name, snapshot_name, device):
    step_assert(step).assert_true(utils.nova2ools_cli.local_volume_create(vm_name, snapshot_name=snapshot_name, device=device))


@step(u'I create volume with resizing for VM instance "(.*)" from snapshot "(.*)" with size "(.*)" as device "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def create_volume_from_snapshot_with_size(step, vm_name, snapshot_name, size, device):
    step_assert(step).assert_true(utils.nova2ools_cli.local_volume_create(vm_name, snapshot_name=snapshot_name, device=device, size=size))

@step(u'I resize device "(.*)" on VM instance "(.*)" to new size of "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def resize_volume(step, device, vm_name, size):
    step_assert(step).assert_true(utils.nova2ools_cli.local_volume_resize(vm_name, device, size))


@step(u'device "(.*)" on VM instance "(.*)" is resized within "(.*)" seconds')
@onfailure(debug.save.log('nova/nova-compute.log'))
def volume_is_resized(step, device, vm_name, timeout):
    step_assert(step).assert_true(utils.nova2ools_cli.wait_local_volume_state(vm_name, device, 'in-use', timeout))
## Here we need status of drive to be returned


@step(u'size of device "(.*)" on VM instance "(.*)" is equal to "(.*)" within "(.*)" seconds')
@onfailure(debug.save.log('nova/nova-compute.log'))
def check_size_volume(step, device, vm_name, size, timeout):
    step_assert(step).assert_equals(utils.nova2ools_cli.get_local_volume_size(vm_name, device) , size.strip())


@step(u'I delete device "(.*)" on VM instance "(.*)"')
@onfailure(debug.save.log('nova/nova-compute.log'))
def delete_volume(step, device, vm_name):
    step_assert(step).assert_true(utils.nova2ools_cli.local_volume_delete(vm_name, device))


@step(u'device "(.*)" on VM instance "(.*)" is deleted within "(.*)" seconds')
@onfailure(debug.save.log('nova/nova-compute.log'))
def volume_is_deleted(step, device, vm_name, timeout):
    step_assert(step).assert_true(utils.nova2ools_cli.wait_local_volume_state(vm_name, device, 'in-use', timeout, expect_fail=True))
...
##  ----------------------------------------------------------
##
@step(u'I use nova2ools-vms list and see "(.*)" user is "(.*)", project is "(.*)", keypair is "(.*)"')
def check_nova2ools_vms_list(step, instance_name, user, project, key_name):
    step_assert(step).assert_true(utils.nova2ools_cli.check_nova2ools_vms_list(instance_name, user=user, project=project, key_name=key_name))
